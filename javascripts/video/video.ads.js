define([], function () {
    ...
    var EXCLUDE_VIDEO_TYPE = "Bandes-annonces"; // The value of the videoType we don't want to play ad
    var MINIMUM_AGE = "under12"; // The value of the value that the userAge need to have to be spotted as underAge
    /**
     ...
     **/
    var state = {};
    var gptAdUnitId, gptAdZone, gptAdKeys, userAge, videoType, adEnabled;
     ...
     */
    var requestAds = function (player) {
        // retreive the adUrl
        var adServerUrl = getAdServerUrl(player);
        console.log("Ad server url: " + adServerUrl);
     ...
     */
    var getAdServerUrl = function (player) {
    ...
    var isUnderAge = function () {
        return (typeof userAge != 'undefined' && userAge === MINIMUM_AGE);
    };
    ...
    var isExcludedVideoType = function () {
        return (typeof videoType != 'undefined' && videoType === EXCLUDE_VIDEO_TYPE);
    };
     ...
     */
    var isAdCuePoint = function (cuePoint,player) {
        var isAd = false;
     ...
     */
    var hasSkipAdCuePoint = function (player) {
        for (var i = 0; i < state.content.cues.length; i++) {
            ...
            if (state.content.seekStart < cueTime && cueTime < state.content.seekEnd) {
                if (isAdCuePoint(state.content.cues[i],player)) {
                    break;
                }
            }
        }
    ...
    adsFramework = function (options) {
        ...
        gptAdUnitId = (options && options.gptAdUnitId) || ""; // The AdUnitId of the site -- needed to build the path of the adRequets
        gptAdZone = (options && options.gptAdZone) || "";// The AdZone of the site -- needed to build the path of the adRequets
        gptAdKeys = (options && options.gptAdKeys) || "";// The Key/value pair to add to the adServer URL

        //TODO: require it
        if (typeof Krux !== "undefined") {
            gptAdKeys += "%26" + "u%3D" + Krux.user;
        }
        userAge = (options && options.userAge) || ""; // The age of the user "under12" if the user is recognize as under 12
        videoType = (options && options.videoType) || ""; // The type of the video
        adEnabled = (options && options.adEnabled) || false;


        player.ima3({
            serverUrl:'',
            requestMode: options.ima3.requestMode,
            timeout: options.ima3.timeout,
            prerollTimeout: options.ima3.prerollTimeout,
            debug: options.ima3.debug,
            loadingSpinner: options.ima3.loadingSpinner
        });
        ...
        player.on('loadedmetadata', function () {
            ...
            tt.oncuechange = function () {
                isAdCuePoint(tt.activeCues[0], this);
            };
     ...
     * @param options
     */
    videojs.plugin('adsPlugin', adsFramework);
});
